...
function displayData() {
    ...
    //V1, Not required for V2 but still leave it here
    page="show";
...
function getMemberBySearch(page) {
    ...
    //Get data from php and form a table
    zixuArray = "";
    input = document.getElementById("name-input").value.toUpperCase();
    if (input == "") {
        getMember(page);
        return;
    }
...
function clickEnter() {
    ...
    if (page == "search") {
        //Temporary disabled for V2 development
        //displayData();

        getMemberBySearch(last_page);
    } else if (page == "show") {
        if (last_focus_id != "") {
            doneEditStudent(last_focus_id);
        }
